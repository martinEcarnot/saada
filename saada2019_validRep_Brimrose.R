...
source("~/Documents/INRA/melanges/2019/saada2019_dalm_mec.R")
# source('~/Documents/INRA/melanges/2019/saada2019_prepa.R')
# source("C:/Users/robot/Documents/Martin/melanges/pre.R")
# source("C:/Users/robot/Documents/Martin/melanges/orthEf2.R")
...
# sp=saada2019_Brimrose_prepa()
# sp$code.champ=as.factor(sp$code.champ)


# sp$Rep=as.factor(sp$Rep)
# save(sp,file='/home/ecarnot/Documents/INRA/melanges/2019/saada2019_sp_Brimrose')

load("~/Documents/INRA/melanges/2019/saada2019_sp_Brimrose")
# load("C:/Users/robot/Documents/Martin/melanges/2019/saada2019_sp_Brimrose")
...
for (i in round(seq(31,ngeno, length.out = 10))) { #1:ngeno) {
  print(i)
  for (j in round(seq(30,ngeno-1, length.out = 10))) { #1:ngeno) { { #
    ...
    if (nlevels(mono2$Genotype1)!=2 | nlevels(mono2$Rep)!=2 | length(unique((mono2$code.champ)))<4) {
      ...
      # fm <- plsda(Xr$xp,Xr$Genotype1,Xu$xpd,Xu$Genotype1, ncomp=comp, da=dalm)
      fm <- plsda(Xr$xp,Xr$Genotype1,Xu$xp,Xu$Genotype1, ncomp=comp, da=saada2019_dalm_mec)
...
ggplot(data = df,aes(x=comp1,y=comp2, col=Genotype1,size=Rep)) +geom_point()
