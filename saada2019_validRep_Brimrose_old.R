...
source("~/Documents/INRA/melanges/2019/saada2019_Brimrose_prepa.R")
...
for (i in 1:ngeno) { #round(seq(31,ngeno, length.out = 10))) { #
  print(i)
  for (j in 1:ngeno) { #round(seq(30,ngeno-1, length.out = 10))) { #1:ngeno) { { #
    ...
    # mono2=orthEf(mo29,mono2,"xp",c(4),5)  # mono2=orthEf(mono2,mono2,"xp",c(1,5,9),1)
    # mono2$xp=mono2$xort
